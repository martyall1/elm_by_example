...
Elm.Paddle = Elm.Paddle || {};
Elm.Paddle.make = function (_elm) {
   "use strict";
   _elm.Paddle = _elm.Paddle || {};
   if (_elm.Paddle.values)
   return _elm.Paddle.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var step = F2(function (event,
   s) {
      return s.isOver ? s : function () {
         switch (event.ctor)
         {case "PaddleDx":
            return _U.replace([["paddledx"
                               ,0.1 * $Basics.toFloat(event._0)]],
              s);
            case "Tick":
            return _U.replace([["x"
                               ,s.x + s.dx * event._0]
                              ...
                              ,["dy"
                               ...
                               -190) < 1 && _U.cmp(s.dy,
                               0) < 0 && (_U.cmp(s.x,
                               s.paddlex - 50) > -1 && _U.cmp(s.x,
                               s.paddlex + 50) < 1) ? -1 * s.dy : s.dy]
                              ,["paddlex"
                               ,A2($Basics.min,
                               A2($Basics.max,
                               s.paddlex + s.paddledx * event._0,
                               -150),
                               150)]
                              ,["isOver"
                               ,_U.cmp(s.y,-200) < 0]],
              s);}
         _U.badCase($moduleName,
         "between lines 101 and 119");
      }();
   });
   var PaddleDx = function (a) {
      return {ctor: "PaddleDx"
             ,_0: a};
   };
   var keyboardSignal = A2($Signal._op["<~"],
   function ($) {
      return PaddleDx(function (_) {
         return _.x;
      }($));
   },
   $Keyboard.arrows);
   ...
   Tick,
   $Time.fps(100));
   var eventSignal = A2($Signal.merge,
   clockSignal,
   keyboardSignal);
   var initialState = {_: {}
                      ,dx: 0.14
                      ...
                      ,x: 0
                      ,y: 0};
   var gameSignal = A2($Signal.foldp,
   step,
   initialState)(eventSignal);
   ...
   var gameOver = A3($Graphics$Element.container,
   400,
   440,
   $Graphics$Element.middle)($Graphics$Element.centered($Text.height(60)($Text.bold($Text.color($Color.red)($Text.fromString("Game Over"))))));
   var ball = F2(function (x,y) {
   ...
   var paddle = function (x) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: x
                                    ,_1: -210})($Graphics$Collage.filled($Color.green)(A2($Graphics$Collage.rect,
      $Basics.toFloat(100),
      $Basics.toFloat(20))));
   };
   var borders = $Graphics$Collage.group(_L.fromArray([$Graphics$Collage.filled($Color.blue)(A2($Graphics$Collage.rect,
                                                      440,
                                                      440))
                                                      ,$Graphics$Collage.moveY(-10)($Graphics$Collage.filled($Color.white)(A2($Graphics$Collage.rect,
                                                      400,
                                                      420)))]));
   var view = function (s) {
      return $Graphics$Element.layers(_L.fromArray([A3($Graphics$Collage.collage,
                                                   440,
                                                   440,
                                                   _L.fromArray([borders
                                                                ,paddle(s.paddlex)
                                                                ,A2(ball,
                                                                s.x,
                                                                s.y)]))
                                                   ,s.isOver ? gameOver : $Graphics$Element.empty]));
   };
   var main = A2($Signal._op["<~"],
   view,
   gameSignal);
   _elm.Paddle.values = {_op: _op
                        ,borders: borders
                        ,paddle: paddle
                        ,ball: ball
                        ,gameOver: gameOver
